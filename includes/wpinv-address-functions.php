...
function wpinv_get_ip_country( $address = null ) {
    ...
    if ( !empty( $real_ip_address ) && $real_ip_address !== "127.0.0.1" ) {
        // Use the PHP GeoIP plugin extensions if at all possible because they are fastest
        if ( function_exists( 'geoip_country_code_by_name') && ( !isset( $wpinv_options['vat_ip_lookup'] ) || $wpinv_options['vat_ip_lookup'] === "geoip" || $wpinv_options['vat_ip_lookup'] === "default" ) ) {
            try {
                $erl = error_reporting();
                error_reporting( $erl & ~E_NOTICE );
                $wpinv_ip_address_country = geoip_country_code_by_name( $real_ip_address );
                
                if ( empty( $wpinv_ip_address_country ) ) {
                    wpinv_error_log( "$real_ip_address ( {$_SERVER['HTTP_HOST']} )", 'Address not found', __FILE__, __LINE__ );
                    
                    $wpinv_ip_address_country = wpinv_get_default_country();
                    
                    $ex = new \Exception( "Address not found" );
                    wpinv_error_log( $ex->getTraceAsString(), 'Address not found', __FILE__, __LINE__ );
                }
        ...
        // Fallback to to the GeoPlugin if possible but this is slower
        else if ( function_exists( 'simplexml_load_file' ) && ( !isset( $wpinv_options['vat_ip_lookup'] ) || $wpinv_options['vat_ip_lookup'] === "geoplugin" || $wpinv_options['vat_ip_lookup'] === "default" ) ) {
            wpinv_error_log( 'simplexml_load_file', '', __FILE__, __LINE__ );
            $xml = simplexml_load_file( "http://www.geoplugin.net/xml.gp?ip=" . $real_ip_address );
            
            if ( !empty( $xml ) && !empty( $xml->geoplugin_countryCode ) ) {
                $wpinv_ip_address_country = (string)$xml->geoplugin_countryCode;
                wpinv_error_log( $wpinv_ip_address_country, 'simplexml_load_file', __FILE__, __LINE__ );
...
function wpinv_getGeoLiteCityFilename() {
...
function wpinv_getGeoLiteCountryFilename() {
    // Get the WordPress upload directory information, which is where we have stored the MaxMind database.
    $upload_dir = wp_upload_dir();
...
function wpinv_getGeoIP2CountryCode($ip_address) {
    // Now get the location information from the MaxMind database.
    try {
        $reader = wpinv_getGeoIP2CountryReader();

        if ( $reader === false ) {
            return wpinv_get_default_country();
        }
        ...
        return empty( $location  ) ? wpinv_get_default_country() : $location;

    } catch(\InvalidArgumentException $e) {
        wpinv_error_log( $e->getMessage(), 'InvalidArgumentException', __FILE__, __LINE__ );
        
        return wpinv_get_default_country();
    } catch(\GeoIp2\Exception\AddressNotFoundException $e) {
        wpinv_error_log( $e->getMessage(), 'AddressNotFoundException', __FILE__, __LINE__ );
...
function wpinv_getGeoIP2CityReader() {
    // Now get the location information from the MaxMind database.
    try {
        wpinv_requireGeoIP2();

        $filename = wpinv_getGeoLiteCityFilename();
        if ( $filename === false ) {
            return false;
        }
...
function wpinv_getGeoIP2CountryReader() {
    // Now get the location information from the MaxMind database.
    try {
        wpinv_requireGeoIP2();

        $filename = wpinv_getGeoLiteCountryFilename();
        if ( $filename === false ) {
            return false;
        }
...
function wpinv_requireGeoIP2() {
    ...
    if ( !class_exists( '\MaxMind\Db\Reader' ) ) {
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/MaxMind/Db/Reader/Decoder.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/MaxMind/Db/Reader/InvalidDatabaseException.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/MaxMind/Db/Reader/Metadata.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/MaxMind/Db/Reader/Util.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/MaxMind/Db/Reader.php' );
    }

    if ( !class_exists( '\GeoIp2\Database\Reader' ) ) {
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/compat/JsonSerializable.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/ProviderInterface.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Exception/GeoIp2Exception.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Exception/AddressNotFoundException.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Exception/AuthenticationException.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Exception/HttpException.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Exception/InvalidRequestException.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Exception/OutOfQueriesException.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Model/AbstractModel.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Model/Country.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Model/City.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Model/ConnectionType.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Model/Domain.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Model/Insights.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Model/Isp.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Record/AbstractRecord.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Record/AbstractPlaceRecord.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Record/City.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Record/Continent.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Record/Country.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Record/Location.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Record/MaxMind.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Record/Postal.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Record/RepresentedCountry.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Record/Subdivision.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Record/Traits.php' );
        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/WebService/Client.php' );

        require_once( WPINV_PLUGIN_DIR . 'includes/libraries/GeoIp2/Database/Reader.php' );
    }
