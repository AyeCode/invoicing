...
class GetPaid_Notification_Email {
	...
	/**
	 * @var array|null Cached emails settings array to avoid redundant calls to wpinv_get_emails().
	 */
	private static $_emails_options = array();
	...
	public function get_option( $key ) {
		...
		if ( is_null( $value ) ) {
			// Prevent load wpinv_get_emails() multiple times.
			if ( empty( self::$_emails_options ) ) {
				self::$_emails_options = wpinv_get_emails();
			}

			if ( ! isset( self::$_emails_options[ $this->id ] ) || ! isset( self::$_emails_options[ $this->id ][ $key ] ) ) {
				return '';
			}

			$value = isset( self::$_emails_options[ $this->id ][ $key ]['std'] ) ? self::$_emails_options[ $this->id ][ $key ]['std'] : '';
		}
